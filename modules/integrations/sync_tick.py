...
Один проход по очереди ``sync_link`` (state=pending, direction=out): попытка выгрузки через
...
Идемпотентно: уже synced-линки не подбираются, а непроверенный адаптер оставляет error
до ручной сверки и повторной отправки.
"""
