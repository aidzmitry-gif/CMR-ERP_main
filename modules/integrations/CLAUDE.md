...
- Этот модуль — **источник шлюзов `onec/stock/registry`**. Базовый `onec` обслуживает read-only справочники; исходящая запись документов остаётся unavailable до отдельного проверенного адаптера. Если модуль выключен, `core.services.onec/stock/registry == None`; операции, которым действительно нужен соответствующий шлюз, отдают 503.
