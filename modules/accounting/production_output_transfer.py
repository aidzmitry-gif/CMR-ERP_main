...
class ProductionOutputTransferInput(Input):
    ...
    def unique_documents(cls, value):
        if any(type(item) is not int or item <= 0 for item in value) or len(value) != len(set(value)):
...
async def _policy(session, org_id: int, month: str, policy_id: int, posting_date: date):
    ...
    policy = await session.scalar(select(Policy).where(
        Policy.organization_id == org_id, Policy.id == policy_id,
    ))
    if policy is None or policy.effective_from > first or policy.production_costing is None:
        raise AccountingError("The saved production policy is not available for this output transfer")
...
async def _document(session, org_id: int, month: str, data: ProductionOutputTransferInput,
                    production, warehouse_gateway):
    review = await preview_output_cost_basis(session, org_id, month, data.policy_id, data.order_id,
                                             data.analytical_order, data.warehouse, production, warehouse_gateway)
    ...
    _, _, policy, settings, accounts = await _policy(session, org_id, month, data.policy_id, data.posting_date)
    target = accounts.get(target_account)
    if target is None:
        raise AccountingError("Finished-goods account is not available on the posting date")
    values = {"warehouse": data.warehouse.strip(), "sku": sku_code.strip(),
              "order": data.analytical_order.strip(), "department": data.department.strip()}
    ...
    if missing:
        raise AccountingError(f"Finished-goods analytics require explicit values: {sorted(missing)}")
    dimensions = {key: values[key] for key in target.required_dimensions}
    try:
        operation_dates = [date.fromisoformat(item["operation_date"]) for item in review["output"]["documents"]]
    ...
    posting = PostingInput(
        source=_posting_source(org_id, data.order_id), source_version=1,
        operation="production_output_transfer", document_date=operation_date,
        operation_date=operation_date, posting_date=data.posting_date, policy_id=policy.id,
        ...
        lines=[
            LineInput(account=target_account, side="debit", amount=amount, dimensions=dimensions, quantity=quantity),
            LineInput(account=settings.wip_account, side="credit", amount=amount,
                      dimensions={"department": data.department.strip(), settings.order_dimension: data.analytical_order.strip()}),
        ],
    )
...
async def prepare_output_transfer(session, org_id: int, month: str, data: ProductionOutputTransferInput,
                                  production, warehouse_gateway):
...
async def confirm_output_transfer(session, org_id: int, month: str, data: ProductionOutputTransferConfirmInput,
                                  actor, event_bus=None, production=None, warehouse_gateway=None):
    ...
    existing = await session.scalar(select(Entry).where(
        ...
        Entry.source_version == 1, Entry.operation == "production_output_transfer",
    ))
    if existing is not None:
        receipt = await session.get(ProductionOutputTransferReceipt, existing.id)
        if receipt is None or existing.digest != data.digest or receipt.basis_digest != data.basis_digest \
                or receipt.actor != actor or receipt.command != data.model_dump(mode="json"):
    ...
    prepared = await prepare_output_transfer(session, org_id, month, data, production, warehouse_gateway)
    ...
    entry = await service.post(session, org_id, posting, actor, event_bus,
                               production_output_transfer=True)
    existing_receipt = await session.get(ProductionOutputTransferReceipt, entry.id)
    if existing_receipt is not None:
        if (existing_receipt.organization_id != org_id or existing_receipt.order_id != data.order_id
                or existing_receipt.basis_digest != data.basis_digest or existing_receipt.digest != data.digest
                or existing_receipt.actor != actor or existing_receipt.command != data.model_dump(mode="json")):
            raise AccountingError("Output transfer already exists with different content")
        return entry
    session.add(ProductionOutputTransferReceipt(
        entry_id=entry.id, organization_id=org_id, order_id=data.order_id, month=month,
        command=data.model_dump(mode="json"), basis=prepared, posting=posting.model_dump(mode="json"),
        basis_digest=data.basis_digest, digest=data.digest, actor=actor,
    ))
    await session.flush()
    return entry
