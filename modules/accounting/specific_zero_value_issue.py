...
async def _command(session, organization_id, document):
    ...
    if policy.inventory_method in {"fifo", "weighted_average"}:
        ...
        if (Decimal(cost["issue_cost_byn"]) != 0
            ...
            raise service.AccountingError("Selected inventory allocation is not entirely zero-cost")
        command = AllocatedZeroValueDisposalCommand(
            command_version=4, operation="inventory_issue", source=document.source,
            ...
            inventory_layers=[ZeroValueInventoryLayer(
            ...
            ) for layer in cost["inventory_layers"]], explanation=document.explanation,
...
async def preview(session, organization_id, document, actor):
    ...
    if isinstance(command, AllocatedZeroValueDisposalCommand):
        receipt = {"source_layers": [{"entry_id": layer.source_entry_id, "line_id": layer.source_line_id,
                                       "quantity": format(layer.quantity, ".6f")}
                                     for layer in command.inventory_layers], "zero_byn": True}
    else:
...
async def implicit_document(session, organization_id, document, *, procurement=None):
    """Select verified origins server-side; browser clients never submit ledger IDs."""
    inventory_issues._guard_production_material_source(document, False)
    ...
    if policy is None or policy.organization_id != organization_id:
        raise service.AccountingError("Zero-value issue requires the active inventory policy")
    if policy.inventory_method in {"fifo", "weighted_average"}:
        if (not cost.get("source_allocation_version")
                or any(Decimal(layer["amount_byn"]) != 0 for layer in cost["inventory_layers"])):
            raise service.AccountingError("Selected inventory allocation is not entirely zero-cost")
        # The immutable V4 command carries the server-selected layers; the client
        # continues to submit only the primary inventory document.
        return document
    if policy.inventory_method != "specific":
        raise service.AccountingError("Zero-value issue supports specific, FIFO, or weighted-average inventory policy")
    target = {"warehouse": document.warehouse, "sku": document.sku, "lot": document.lot}
...
async def implicit_confirm(session, organization_id, document, basis_digest, digest, actor, *, procurement=None):
    ...
    if existing is not None:
        saved = parse_zero_value_command(existing.command)
        if isinstance(saved, AllocatedZeroValueDisposalCommand):
            if saved.document != document.model_dump(mode="json"):
...
async def confirm(session, organization_id, document, basis_digest, digest, actor):
    ...
    if existing is not None:
        saved = parse_zero_value_command(existing.command)
        if isinstance(saved, AllocatedZeroValueDisposalCommand):
            unchanged = (existing.actor == actor and existing.basis_digest == basis_digest and existing.digest == digest
                         and saved.document == document.model_dump(mode="json"))
        else:
            layer, = saved.inventory_layers
            unchanged = (
