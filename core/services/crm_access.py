...
async def get_crm_access(
    ...
    return await get_crm_access_for_user(session, user)


async def get_crm_access_for_user(session: AsyncSession, user: CurrentUser) -> CrmAccess:
    """Resolve persistent visibility for commands without the HTTP liveness bypass."""
    ...
    app_user = (await session.execute(select(User).where(identity_filter).execution_options(populate_existing=True))).scalar_one_or_none()
    if app_user is None:
