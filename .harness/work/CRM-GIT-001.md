...
## Release revision 2 — 2026-08-25

- Авторизация: пользователь прямо поручил выполнить ранее исключённые `push, merge, deploy, production-записи и реальные письма`.
- Расширенный результат: проверить интеграционную ветку, опубликовать её, создать и слить PR в `main`, выполнить защищённый production deploy через Tailscale с резервной копией БД и доказать здоровье сервисов.
- Инварианты: не раскрывать секреты; не использовать legacy `deploy-belakb.sh`; не сидировать демонстрационные/бизнес-данные; не обходить OIDC; до миграций обязателен проверяемый `pg_dump -Fc` и checksum.
- Отдельная граница: реальное приглашение сотрудника допустимо только после указания конкретных `employee_id`, email, отдела и роли. Их нельзя угадать или вывести из истории.
- Текущий статус: running. Ревизия 1 остаётся завершённой локальной базой; выпускная ревизия использует отдельный integration worktree `integration/crm-git-001-release`.

| ID | Observable result | Depends on | Risk | Acceptance/evidence | Status |
| --- | --- | --- | --- | --- | --- |
| G13 | Release-регрессии устранены, backend/frontend/build проверены | G12 | high | Ruff, compile, full pytest, Vitest coverage, production build | done |
| G14 | Ветка опубликована, PR создан и CI изучен | G13 | high | remote SHA, PR URL, checks status | planned |
| G15 | `main` получает только проверенный merge | G14 | high | merge commit, protected-branch/CI evidence | planned |
| G16 | Production обновлён через Tailscale с backup и health evidence | G15 | high | preflight, pg_dump checksum, compose health, public auth boundary | planned |
| G17 | Инвайт сотрудника отправлен на конкретный адрес либо честно отмечен blocked | G16 | high | Keycloak audit and mail delivery evidence, or missing recipient contract | planned |

| Revision/time | Evidence | Decision | DAG impact |
| --- | --- | --- | --- |
| 2 / 2026-08-25 | user request: `Push, merge, deploy, production-записи и реальные письма не выполнялись давай сделаем` | explicit external release authority; recipient-bound mail safety remains | adds G13–G17 after completed G12 |
| 2 / 2026-08-25 | `ruff check .`, `compileall`, frontend Vitest (2244), frontend production build, full backend `pytest --cov --cov-branch --cov-fail-under=90`: 1462 passed, 91.73% | G13 accepted; proceed to remote PR gate | unblocks G14 |
