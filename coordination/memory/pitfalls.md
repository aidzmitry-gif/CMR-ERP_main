...
2c. **СИМПТОМ: работа сделана, но не закоммичена — воркер «ждал уведомления» и умер.**
   Причина: длинную команду (полный `pytest`) запустили ФОНОМ, а потом закончили ход со
   словами «дождусь уведомления». У тебя нет следующего хода: ты живёшь в `claude --print`,
   и как только ход закончен, процесс завершается, а фоновая команда убивается через ~5 секунд.
   Результат — worktree с несохранённым файлом и без баннера. (Поймано 25.07.2026 на
   `wms-inventory-tests`: 6 готовых тестов остались незакоммиченными.)
   → **Никогда не запускай фоновые команды и не жди уведомлений.** Гоняй `pytest` ПРЯМЫМ
   вызовом и дожидайся вывода в том же ходе. И **коммить сразу, как только файл готов**, —
   не откладывай коммит до финальной проверки.
   ⚠️ Но помни про потолок: команда дольше **10 минут** принудительно уходит в фон, и ты
   снова остаёшься без результата. Полный `pytest -q` этого проекта в него НЕ укладывается.
   → Гоняй **свой** файл и соседние тесты своего модуля (`pytest tests/test_<модуль>*.py -q`),
   а полный прогон — забота оркестратора. Если задание требует полного прогона, это ошибка
   задания: доложи `КООРД: NEEDS-ARB`, а не пытайся обойти фоном.
