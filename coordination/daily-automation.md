...
> **⛔ ОТКЛЮЧЕНО 2026-07-16** (решение владельца, «вариант 2» аудита сессий 10–16.07):
> обе задачи переведены в Disabled — дайджест не читался (провал крона не замечали 8 дней),
> а headless `claude --print` заблокирован организацией. Вместо рассылки статус теперь
> впрыскивается при старте каждой сессии SessionStart-хуком `scripts/session_health.py`
> (dev-серверы, свежесть readiness/STATUS-меток, ACTIVE-SESSIONS).
> Вернуть при желании: `schtasks /Change /TN "CRM-tg-digest" /ENABLE` и
> `schtasks /Change /TN "CRM-daily-review" /ENABLE` (файлы задач и скрипты не удалялись).
