...
Единственность открытой версии теперь гарантирована и на уровне БД: миграция 0084
добавила partial-unique индексы `UNIQUE (<natural_key>) WHERE end_date IS NULL` на всех
четырёх таблицах (`uq_ref_currency_rate_open`, `uq_ref_vat_rate_open`, `uq_ref_tnved_open`,
`uq_ref_sku_version_open`). Гонка двух параллельных транзакций (обе не увидели открытую
версию друг друга и обе сделали INSERT) теперь невозможна: вторую открытую версию по тому
же ключу отклонит Postgres (IntegrityError) — прикладная проверка `add_version` осталась
первым эшелоном. Индекс — источник истины ТОЛЬКО для Postgres (SQLite-dev поднимает схему
через ORM `create_all`, минуя Alembic), поэтому его отклонение проверяется на реальном
Postgres в `tests/integration/test_scd2_partial_unique.py`; тесты ниже фиксируют
последовательный прикладной инвариант (SQLite в памяти).
"""
