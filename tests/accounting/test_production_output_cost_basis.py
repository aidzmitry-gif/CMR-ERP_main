...
def policy(*, target=None):
    ...
    return Policy(id=7, organization_id=11, effective_from=date(2026, 10, 1),
                  reference="Reviewed production policy", production_costing=config)
...
def line(line_id, *, side="debit", amount="100.00", order="A"):
    return {"entry_id": 10, "line_id": line_id, "source": "production-input", "account": "20",
            "posting_date": "2026-10-04", "side": side, "amount_byn": amount,
            "dimensions": {"department": "SHOP", "order": order}, "opening": False}


def output(*, operation_date="2026-10-10", planned="2", confirmed="2.00", accepted="2.00", rejected="0.00", pending="0.00"):
    return {"planned_quantity": planned, "confirmed_quantity": confirmed, "accepted_quantity": accepted,
            "rejected_quantity": rejected, "pending_quantity": pending, "unit": "шт",
            "documents": [{"document_id": 1, "operation_date": operation_date, "quantity": confirmed,
                           "state": "accepted", "receipt_id": 2, "accepted_quantity": accepted,
...
async def prepare(monkeypatch, output, *, target=None, lines=None):
    ...
    monkeypatch.setattr(production_output_cost, "cost_sources",
...
async def test_negative_wip_is_rejected(monkeypatch):
    with pytest.raises(AccountingError, match="negative"):
        await prepare(monkeypatch, output(planned="1", confirmed="1.00", accepted="1.00"),
...
async def test_preview_endpoint_is_company_scoped_and_private(client, book, monkeypatch):
    async def fake_preview(*args, **kwargs):
        return {"organization_id": book[0], "month": "2026-10", "policy_id": book[1], "scope": "production_output_cost_basis"}
    ...
    response = await client.get(
        ...
        params={"policy_id": book[1], "order_id": 42, "order_analytics": "A", "warehouse": "Главный"},
    )
