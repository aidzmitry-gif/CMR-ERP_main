...
from modules.sales.models import CallLog, CrmClient, Deal, Stage
...
async def call_factory():
    ...
    url = make_url(raw)
    if (url.host, url.port, url.database) != ("127.0.0.1", 15440, "crm_ready_calls") and not is_joint_test_url(url):
