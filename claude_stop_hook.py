...
их ведёт claude_quality_hook.py), — и:
  ...
  • при успехе очищает ВСЕ реестры, которые только что прогнал через гейт — свой и
    сабагентские (следующий ход начинается с чистого листа). Иначе сабагентские не
    удалял бы никто: SubagentStop не зарегистрирован, они копились бы до 20 на сессию,
    а один красный файл от мёртвого сабагента блокировал бы ход родителя навсегда.
    Живой фоновый сабагент просто создаст свой файл заново; удаление обёрнуто в
    try/except OSError (на Windows файл может держать другой процесс — WinError 32).
...
def _ledger(proj: Path, sid: str) -> Path:
    """Свой (без agent-суффикса) реестр — единственный, который этот хук удаляет."""
...
def main() -> int:
    ...
    ledger = _ledger(proj, sid)  # свой — единственный, который удаляем
    ...
    # Успех — чистим ВСЕ реестры, которые только что прогнали через гейт (свой + сабагентские).
    # Иначе сабагентские не удаляет никто (SubagentStop не зарегистрирован), они копятся до 20
    # на сессию, ruff на каждом ходу перелинчивает всё старое, а один красный файл из мёртвого
    # сабагента блокирует ход родителя навсегда. Реестр = «тронуто с последнего зелёного гейта»,
    # поэтому после зелёного его и обнуляем. Фоновый сабагент, если ещё жив, создаст файл заново.
    for _lg in ledgers:
        try:
