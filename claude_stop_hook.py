...
их ведёт claude_quality_hook.py), — и:
  ...
  • при успехе очищает СВОЙ реестр (следующий ход начинается с чистого листа).
    Реестры сабагентов НЕ трогает — фоновый сабагент может пережить ход родителя
    и дописывать в свой файл дальше; удалить чужой на Windows — PermissionError
    (WinError 32), если файл держит другой процесс.
