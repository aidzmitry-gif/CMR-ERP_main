...
describe("workingMinutesBetween", () => {
  ...
  it("несколько дней: хвост + полный день + утро (будни, без воскресенья)", () => {
    // Пн 13-го 17:00→18:00 (60) + весь Вт 14-й (540) + Ср 15-го 9:00→10:00 (60)
    expect(workingMinutesBetween(d(13, 17, 0), d(15, 10, 0))).toBe(660);
  });

  it("воскресенье — выходной: суббота считается, воскресенье пропускается", () => {
    // 2026-07-11 суббота (рабочая), 07-12 воскресенье (выходная): Сб 17:00→Вс 12:00
    // = Сб 17-18 (60) + всё воскресенье 0. Пн 07-13 включается снова.
    expect(workingMinutesBetween(d(11, 17, 0), d(12, 12, 0))).toBe(60);
    // Сб 17:00 → Пн 10:00 = Сб хвост 60 + Вс 0 + Пн утро 60 = 120
    expect(workingMinutesBetween(d(11, 17, 0), d(13, 10, 0))).toBe(120);
    // весь воскресный день — ноль
    expect(workingMinutesBetween(d(12, 9, 0), d(12, 18, 0))).toBe(0);
  });
