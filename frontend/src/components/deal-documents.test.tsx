...
describe("DealDocuments — сумма выпущенного счёта", () => {
  it.each(["posted", "paid"])("сумма с НДС 360 берётся из оригинала %s, оплата и остаток неизвестны", async (status) => {
    ...
    expect(screen.getAllByText(/(?:Под заказ|Выпущен под заказ) — товар не зарезервирован/)).toHaveLength(2);
  });
  ...
  it("черновик замены сохраняет старый итог и не допускает неподдержанный выпуск", async () => {
    ...
    mock(api.fetchDocuments).mockResolvedValue([old, next]);
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    render(<DealDocuments dealId="1" />);
    await moneySummary().findByText("360 BYN");
    expect(screen.queryByRole("button", { name: "Выпустить версию" })).toBeNull();
    expect(screen.getAllByText("Замена счёта пока недоступна; оригинал и история сохраняются.").length).toBeGreaterThan(0);
    expect(moneySummary().getByRole("link")).toHaveAttribute("href", "/api/sales/documents/50/render");
    expect(moneySummary().getAllByText("Нет данных")).toHaveLength(2);
    expect(fetchMock.mock.calls.filter(([, options]) => options?.method === "POST")).toHaveLength(0);
  });

  it("под заказ после резервирования не отображается как незарезервированный", async () => {
    mock(api.fetchDocuments).mockResolvedValue([{ ...issuedInvoice, reserve_status: "reserved", valid_until: "2099-12-31" }]);
    render(<DealDocuments dealId="1" />);
    await screen.findByText(/В резерве · до/);
    expect(screen.queryByText(/товар не зарезервирован/)).toBeNull();
  });
