...
import { claimClientBinding, previewClientBinding, type BindingClaim, type BindingPreview, type ClientBinding } from "@/lib/client-document-register-api";
...
function BindingCard({ org, dealId, onBound }: { org: string; dealId: string; onBound?: () => void }) {
  ...
  return <section aria-label="Подтверждение клиента сделки" className="space-y-3 rounded-xl border border-line bg-surface p-4">
    ...
    <label className="block text-sm">Точный ID клиента из MDM<Input aria-label="ID клиента для привязки" inputMode="numeric" value={client} disabled={!!prepared || !!confirmed || !registerId(org) || !registerId(dealId)} onChange={(e) => changeClient(e.target.value)} /></label>
