    ...
    <div className="mt-4 rounded-xl border border-slate-200 p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 font-semibold text-ink">
          <UserRound size={18} className="text-brand-600" /> Контакты
      ...
      <ul className="mt-3 space-y-2">
        ...
        {items.map((c) => (
          <li key={c.id} className="flex items-center gap-3 rounded-lg bg-slate-50 px-3 py-2">
            ...
            {!c.is_primary && (
              <button
                ...
                className="shrink-0 text-slate-300 hover:text-amber-500 disabled:opacity-60"
              >
                <Star size={16} />
              </button>
            )}
      ...
      {adding && (
        <div className="mt-3 space-y-2 rounded-lg border border-slate-200 p-3">
          ...
          <button
            ...
            className="w-full rounded-lg bg-brand py-2 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-60"
          >
            Сохранить контакт
          </button>
        </div>
