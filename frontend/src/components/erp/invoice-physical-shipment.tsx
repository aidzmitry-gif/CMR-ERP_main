...
function Panel({scope}:{scope:Scope}){
  ...
  return <section aria-label="Фактическая отгрузка счёта" className="space-y-4 rounded-xl border border-line bg-surface p-4">
    ...
    {pending&&<div className="space-y-2 rounded border border-amber-300 p-3"><p role="status">Запрос сохранён. До проверки результата не создавайте новую отгрузку. Повтор использует прежние строки, дату и количества.</p><p>Дата {pending.body.operation_date} · {pending.body.evidence}</p><ul>{pending.body.lines.map(l=><li key={lineKey(l)}>Строка {l.line_no} · {l.warehouse} · {l.qty}</li>)}</ul><Button disabled={busy||storageError} onClick={()=>void submit(pending,true)}>Проверить результат и повторить тот же запрос</Button></div>}
    {!loading&&!pending&&!storageError&&!review&&<Button variant="secondary" disabled={busy} onClick={()=>void reload()}>Обновить основания отгрузки</Button>}
    ...
    {review&&!pending&&<div className="space-y-3 rounded border border-accent p-3"><h3 className="font-semibold">Подтверждение фактического выбытия</h3><p>{review.body.operation_date} · {review.body.evidence}</p><ul>{review.body.lines.map(l=><li key={lineKey(l)}>Строка {l.line_no} · {review.skus.find(s=>lineKey(s)===lineKey(l))?.sku_code} · {l.warehouse} · {l.qty}</li>)}</ul><p>После подтверждения будут записаны расход товара и уменьшение резерва.</p><Button disabled={busy} onClick={()=>void submit(review)}>Подтвердить фактическую отгрузку</Button><Button variant="secondary" disabled={busy} onClick={()=>setReview(null)}>Вернуться к количествам</Button></div>}
