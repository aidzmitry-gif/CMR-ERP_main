...
type Calculation = { net_amount_byn?: string; vat_amount_byn?: string; gross_amount_byn?: string; cost: { method?: string; basis_digest: string; book_quantity: string; book_value_byn: string; remaining_quantity: string; remaining_value_byn: string; inventory_layers?: { lot: string; quantity: string; amount_byn: string; dimensions: Record<string, string> }[]; evidence: { entry_id: number; line_id: number; source: string; source_version: number; amount_byn: string; quantity: string }[] }; digest: string; posting: { lines: { side: string; account: string; amount: string; quantity: string | null; dimensions: Record<string, string> }[] } };
export function AccountingInventoryIssue({ org, date, policyId, inventoryMethod = "specific", accounts, onBusyChange, onPosted, onDate, onEntry, sale = false }: { org: string; date: string; policyId?: number; inventoryMethod?: "specific" | "fifo" | "weighted_average"; accounts: Account[]; onBusyChange: (busy: boolean) => void; onPosted: () => void; onDate: (value: string) => void; onEntry?: (id: number) => void; sale?: boolean }) {
  ...
  async function execute(confirm: boolean) {
    ...
    try {
      ...
      if (confirm) { setPrepared(null); setNotice(sale ? `Продажа № ${result.id} проведена.` : `Бухгалтерское списание № ${result.id} проведено.`); onPosted(); }
  ...
  const inventory = accounts.filter((account) => account.category === "asset" && !account.cash && account.quantity_tracking && (sale ? belongs(account.code, "41") : /^(10|41)(\.|$)/.test(account.code)));
  ...
  return <section aria-label={sale ? "Бухгалтерская продажа товаров" : "Бухгалтерское списание запасов"} className="space-y-4 rounded-xl border border-line bg-surface p-4">
    ...
    {current && <div className="space-y-2 rounded border border-accent p-3">
      {sale && <p>Без НДС: {current.calculation.net_amount_byn} BYN · НДС: {current.calculation.vat_amount_byn} BYN · К оплате: {current.calculation.gross_amount_byn} BYN</p>}
      ...
      {current.calculation.posting.lines.map((line, index) => <p key={index}>{line.side === "debit" ? "Дт" : "Кт"} {line.account} · {line.amount} BYN{line.quantity ? ` · Количество ${line.quantity}` : ""} · {JSON.stringify(line.dimensions)}</p>)}
      ...
      <details><summary>Проводки, использованные в расчёте</summary>{current.calculation.cost.evidence.map((row) => <p key={row.line_id} className="break-words"><button type="button" className="text-accent underline" disabled={busy || !onEntry} onClick={() => onEntry?.(row.entry_id)}>Операция № {row.entry_id} · {row.source}</button> · версия {row.source_version} · {row.amount_byn} BYN · количество {row.quantity}</p>)}</details>
