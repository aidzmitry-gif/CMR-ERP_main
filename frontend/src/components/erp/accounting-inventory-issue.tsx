...
type ZeroSaleCommand = { command_version?: number; inventory_layers?: { source_entry_id: number; source_line_id: number; quantity: string }[] };
type Calculation = { kind?: "monetary"; net_amount_byn?: string; vat_amount_byn?: string; gross_amount_byn?: string; zero_value_command?: ZeroSaleCommand; cost: Cost; digest: string; posting: { lines: { side: string; account: string; amount: string; quantity: string | null; dimensions: Record<string, string> }[] } } | { kind: "quantity_only_receipt"; cost: Cost; digest: string; posting: null; receipt: { source_layer?: SourceLayer; source_layers?: SourceLayer[]; zero_byn: true } };
export function AccountingInventoryIssue({ org, date, policyId, inventoryMethod = "specific", accounts, onBusyChange, onPosted, onDate, onEntry, sale = false }: { org: string; date: string; policyId?: number; inventoryMethod?: "specific" | "fifo" | "weighted_average"; accounts: Account[]; onBusyChange: (busy: boolean) => void; onPosted: () => void; onDate: (value: string) => void; onEntry?: (id: number) => void; sale?: boolean }) {
  ...
  const quantitySources = current?.calculation.kind === "quantity_only_receipt"
    ? current.calculation.receipt.source_layers ?? (current.calculation.receipt.source_layer ? [current.calculation.receipt.source_layer] : [])
    : current?.calculation.zero_value_command?.command_version === 4
      ? (current.calculation.zero_value_command.inventory_layers ?? []).map((layer) => ({ entry_id: layer.source_entry_id, line_id: layer.source_line_id, quantity: layer.quantity }))
      : [];
  const zeroCommercialSale = sale && current?.calculation.kind !== "quantity_only_receipt" && current?.calculation.zero_value_command?.command_version === 4;
