...
type Cost = { method?: string; basis_digest: string; book_quantity: string; book_value_byn: string; remaining_quantity: string; remaining_value_byn: string; issue_cost_byn?: string; inventory_layers?: { lot: string; quantity: string; amount_byn: string; dimensions: Record<string, string> }[]; evidence: Evidence[] };
type Calculation = { kind?: "monetary"; net_amount_byn?: string; vat_amount_byn?: string; gross_amount_byn?: string; cost: Cost; digest: string; posting: { lines: { side: string; account: string; amount: string; quantity: string | null; dimensions: Record<string, string> }[] } } | { kind: "quantity_only_receipt"; cost: Cost; digest: string; posting: null; receipt: { source_layer: { entry_id: number; line_id: number; quantity: string }; zero_byn: true } };
export function AccountingInventoryIssue({ org, date, policyId, inventoryMethod = "specific", accounts, onBusyChange, onPosted, onDate, onEntry, sale = false }: { org: string; date: string; policyId?: number; inventoryMethod?: "specific" | "fifo" | "weighted_average"; accounts: Account[]; onBusyChange: (busy: boolean) => void; onPosted: () => void; onDate: (value: string) => void; onEntry?: (id: number) => void; sale?: boolean }) {
  ...
  return <section aria-label={sale ? "Бухгалтерская продажа товаров" : "Бухгалтерское списание запасов"} className="space-y-4 rounded-xl border border-line bg-surface p-4">
    ...
    {current && <div className="space-y-2 rounded border border-accent p-3">
      ...
      {current.calculation.kind === "quantity_only_receipt" ? <><p role="status">Себестоимость 0.00 BYN: проводка не создаётся. Количество {current.calculation.receipt.source_layer.quantity} отражается квитанцией.</p><p>Источник количества: операция № {current.calculation.receipt.source_layer.entry_id}, строка № {current.calculation.receipt.source_layer.line_id}.</p></> : current.calculation.posting.lines.map((line, index) => <p key={index}>{line.side === "debit" ? "Дт" : "Кт"} {line.account} · {line.amount} BYN{line.quantity ? ` · Количество ${line.quantity}` : ""} · {JSON.stringify(line.dimensions)}</p>)}
