...
export function AccountingView() {
  ...
  return <div className="min-w-0 w-full space-y-5 p-6 lg:pr-24 text-ink">
    ...
    <div className="flex flex-wrap gap-4 rounded-xl border border-line bg-surface p-4">
      <label className="min-w-0 basis-64 flex-1 text-sm">Организация<Select className="min-w-0" aria-label="Организация" value={org} disabled={locked} onChange={(e) => { generation.current += 1; setReport(null); setAccountActivity(null); setDetail(null); setPreview(null); setPrepared(null); setNotice(""); setLines([{ account: "", side: "debit", amount: "", dimensions: {} }, { account: "", side: "credit", amount: "", dimensions: {} }]); setOrg(e.target.value); }}><option value="">Выберите организацию</option>{organizations.map((o) => <option key={o.id} value={o.id}>{o.name} · {o.unp}</option>)}</Select></label>
    ...
    <nav aria-label="Разделы бухгалтерии" className="flex flex-wrap gap-2">{[["home", "Рабочее место"], ["expenses", "Контроль расходов"], ["reports", "ОСВ и отчёты"], ["accounts", "План счетов"], ["entry", "Ручная операция"], ["bank", "Банк"], ["bank-import", "Импорт выписки"], ["payroll-accruals", "Начисления зарплаты"], ["payroll-statutory", "Удержания и взносы"], ["invoice-settlements", "Оплаты счетов"], ["seller-profiles", "Реквизиты продавца"], ["input-vat", "Входной НДС"], ["output-vat", "Исходящий НДС"], ["foreign-trade", "ВЭД"], ["fx-revaluation", "Валюты"], ["fixed-assets", "ОС и амортизация"], ["repairs", "Ремонты"], ["production", "Производство"], ["reconciliation", "Сверка ОСВ"], ["inventory-issue", "Списание запасов"], ["sale", "Продажа товаров"], ["controls", "Управление книгой"]].map(([id, label]) => <Button key={id} disabled={locked} variant={tab === id ? "primary" : "secondary"} onClick={() => setTab(id)}>{label}</Button>)}</nav>
    {tab === "expenses" && <ExpenseControl org={org} />}
    ...
    {tab === "fx-revaluation" && <AccountingFxRevaluation org={org} month={operationDate.slice(0, 7)} policy={policies.filter((p) => p.effective_from <= operationDate).sort((a, b) => b.effective_from.localeCompare(a.effective_from))[0]} disabled={locked} onEntry={(id) => void openEntry(id)} />}
    ...
    {tab === "bank-import" && <AccountingBankImport key={org} org={org} accounts={accounts} policyId={policies.filter((p) => p.effective_from <= operationDate).sort((a, b) => b.effective_from.localeCompare(a.effective_from))[0]?.id} date={operationDate} onDate={(value) => changeDate("operation", value)} onPosted={() => void refresh()} />}
