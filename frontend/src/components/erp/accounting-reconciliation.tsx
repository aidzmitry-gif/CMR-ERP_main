...
export function AccountingReconciliation({ org, start, end }: { org: string; start?: string; end?: string }) {
  ...
  async function compare() {
    ...
    try {
      ...
      setProtocol(value); setConfirmKey(value.cutover_ready ? newKey() : ""); setQueueKey(value.cutover_ready ? "" : newKey()); setEvidence(""); setResponsible(""); setQueueEvidence("");
    } catch (e) { setError(e instanceof Error ? e.message : "Не удалось выполнить сверку."); }
    finally { setBusy(false); }
  }

  async function confirm() {
    if (!org || !left || !right || !protocol?.cutover_ready || protocol.accepted_by_accountant || !confirmKey || evidence.trim().length < 10 || confirmBusy || busy || queueBusy || queueRetry) return;
    ...
    try {
      ...
      if (!isRecord(result) || String(result.organization_id) !== org || result.request_key !== confirmKey || result.accepted_by_accountant !== true || result.cutover_ready !== true) throw new Error("Сервер вернул неподтверждённый протокол сверки.");
  ...
  async function queue() {
    if (!org || !left || !right || !protocol || protocol.cutover_ready || busy || confirmBusy || queueBusy || (!queueRetry && (!queueKey || !responsible.trim() || queueEvidence.trim().length < 10))) return;
