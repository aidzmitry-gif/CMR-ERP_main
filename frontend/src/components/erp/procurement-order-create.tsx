...
type Line = { sku_code: string; qty: string; goods_value_byn: string; weight: string; volume: string };
...
const codes: Record<string, string> = { request_basis_changed: "Заявка изменилась. Проверьте её заново.", request_basis_unavailable: "Заявка недоступна в выбранном юрлице.", request_already_linked: "Заявка уже связана с заказом.", command_abandoned: "Неисполненная попытка закрыта. Поздний запрос не создаст заказ." };
...
function validCommand(c: Command) {
  ...
  return uuid.test(c.request_key) && !!d && exactKeys(d, ["supplier", "eta_date", "freight_byn", "lines"]) && textValid(d.supplier, 255) && decimal(d.freight_byn, 2) && textValid(c.ownership_evidence, 1000) &&
    ...
    d.lines.every(x => !!x && exactKeys(x, ["sku_code", "qty", "goods_value_byn", "weight", "volume"]) && textValid(x.sku_code, 64) && decimal(x.qty, 2, true) && decimal(x.goods_value_byn, 2) && decimal(x.weight, 3) && decimal(x.volume, 4)) &&
...
async function validOutcome(o: Outcome | undefined, org: number, principal: string, command: Command) {
  ...
  if (o.outcome !== "created" || !exactKeys(o, ["organization_id", "request_key", "principal", "outcome", "order_id", "ownership_id", "number", "status", "supplier", "eta_date", "freight_byn", "lines", "request_id", "request_ownership_id", "link_id", "request_snapshot"]) ||
      ...
      !o.lines.every((line, i) => !!line && exactKeys(line, ["id", "sku_code", "qty", "goods_value_byn", "weight", "volume"]) && positiveId(line.id) && Object.entries(command.document.lines[i]).every(([key, value]) => line[key as keyof Line] === value))) return false;
...
function OrderWorkspace({ org }: { org: number }) {
  ...
  function dispatch(action: "new" | "retry" | "reconcile") {
    void run(async token => {
      ...
      if (action === "new") {
        ...
        const c: Command = { request_key: crypto.randomUUID(), document: { supplier: trim(supplier), eta_date: eta || null, freight_byn: trim(freight), lines: lines.map(x => Object.fromEntries(Object.entries(x).map(([k, v]) => [k, trim(v)])) as Line) }, ownership_evidence: trim(evidence), request_basis: mode === "request" ? { request_id: Number(requestId), expected_stage: "approval", expected_hash: basis!.basis_hash, link_evidence: trim(linkEvidence) } : null };
  ...
  function revise() {
    void run(async token => {
      ...
      setSupplier(c.document.supplier); setEta(c.document.eta_date ?? ""); setFreight(c.document.freight_byn); setLines(c.document.lines); setEvidence(c.ownership_evidence);
  ...
  return <div className="space-y-4">{error && <p role="alert">{error}</p>}{identity && !identity.can_manage && <p>Доступен просмотр. Создание и изменение требуют прав главного бухгалтера.</p>}
    ...
    <fieldset disabled={formDisabled} className="space-y-3 rounded border border-line p-4"><legend>Новый заказ</legend>
      ...
      {lines.map((line, index) => <div className="grid gap-2 rounded border border-line p-3 md:grid-cols-5" key={index}>{(["sku_code", "qty", "goods_value_byn", "weight", "volume"] as const).map(field => <label key={field}>{({ sku_code: "Код SKU", qty: "Количество", goods_value_byn: "Стоимость товара, BYN", weight: "Вес, кг", volume: "Объём, м³" })[field]} {index + 1}<Input value={line[field]} onChange={e => setLines(a => a.map((x, i) => i === index ? { ...x, [field]: e.target.value } : x))} /></label>)}<Button disabled={lines.length === 1} variant="secondary" onClick={() => setLines(a => a.filter((_, i) => i !== index))}>Удалить позицию {index + 1}</Button></div>)}
