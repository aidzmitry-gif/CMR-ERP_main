...
const reads: Record<string, unknown> = {
  ...
  "crm-staff": [],
  employees: [{ id: 17, full_name: "Тестовый руководитель", department: "Руководство", position: "Директор", status: "active" }],
};
...
describe("invitation reads", () => {
  it("offers a separate director mode using authenticated HR reads", async () => {
    const fetchMock = mockReads();
    render(await EmployeeInvitationsPage({ searchParams: Promise.resolve({ scope: "erp" }) }));
    expect(screen.getByLabelText("Сотрудник")).toHaveTextContent("Тестовый руководитель");
    expect(screen.getByRole("link", { name: "Руководители и другие отделы" })).toHaveAttribute("aria-current", "page");
    expect(fetchMock.mock.calls.some(([url]) => url.endsWith("/hr/employees"))).toBe(true);
    expect(fetchMock.mock.calls.some(([url]) => url.endsWith("/crm-staff"))).toBe(false);
  });

  it("rejects an operator forging general scope despite the director cookie", async () => {
    const fetchMock = mockReads("access", response({ current_roles: ["crm_invitation_operator"] }));
    render(await EmployeeInvitationsPage({ searchParams: Promise.resolve({ scope: "erp" }) }));
    expect(screen.getByRole("alert")).toHaveTextContent("только руководителю");
    expect(screen.queryByRole("button", { name: "Проверить и продолжить" })).not.toBeInTheDocument();
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it.each([401, 403, 500])("blocks general invitations when HR returns %i", async (status) => {
    mockReads("employees", response({}, status));
    render(await EmployeeInvitationsPage({ searchParams: Promise.resolve({ scope: "erp" }) }));
    expect(screen.getByRole("alert")).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Проверить и продолжить" })).not.toBeInTheDocument();
  });

  it("rejects malformed HR rows instead of allowing duplicate creation from an empty list", async () => {
    mockReads("employees", response([{ id: "17", full_name: "Broken" }]));
    render(await EmployeeInvitationsPage({ searchParams: Promise.resolve({ scope: "erp" }) }));
