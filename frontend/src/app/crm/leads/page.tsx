...
export default async function LeadsPage() {
  ...
  const { state: loadState, leads, visibility } = await loadLeadsServer(role, token, await currentDevUsername());
  return (
    <AppShell crumbs={["CRM", "Лиды"]}>
      <LeadsWorkspace initialLeads={leads} initialLoadState={loadState} ownOnly={visibility === "own"} />
    </AppShell>
  );
}
