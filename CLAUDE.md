...
- ⚠️ **С Claude Code 2.1.215 ассистент НЕ запускает `/code-review` и `/verify` по своей
  инициативе** — их зовёт оператор или скилл `ship-cycle` явной командой. «Само не проверилось»
  теперь = «никто не позвал».
- `/code-review` с 2.1.218 идёт **фоновым сабагентом**: не забивает контекст сессии, результат
  приходит уведомлением. Стоящие следом слэш-команды остаются его целью ревью.
- `/code-review ultra` — облачное мульти-агентное ревью ветки или PR. Запускает ТОЛЬКО оператор,
  оно тарифицируется отдельно; ассистент его вызвать не может. `/ultrareview` — устаревший алиас.
...
- **Модель:** Haiku 4.5 (механика: поиск/чтение/скаффолд/форматирование) → Sonnet 5 (ревью/код/
  порт/тесты) → **Opus 5** `claude-opus-5` (деньги/безопасность/схема-миграции/контракты/судья).
  **Fable 5 — ВНЕ тиринга** (флагман $10/$50, 10× Haiku): не дефолт, звать точечно, только если
  Opus не тянет. Opus 4.8/4.7 — legacy, в новые ТЗ не писать.
- **⚠️ `/fast` на Opus 5 стоит $10/$50 — цена Fable, вдвое дороже обычного Opus.** Включать только
  ради латентности в живой работе, никогда — на фоновых воркерах и длинных прогонах.
- **Effort (Faster↔Smarter) — ОРТОГОНАЛЕН модели:** `low` механика · `medium` фича · `high` ревью/
  ...
  `Opus+high..xhigh` T1-зона. **У Opus 5 и Sonnet 5 дефолт effort = `high`** — на механике его надо
  ЯВНО опускать до `low`, иначе платишь как за ревью.
...
- **Браузер/скриншоты — только Playwright MCP** *(замер 2026-07-25, CC 2.1.219)*. Встроенный
  Browser pane ненадёжен для скриншотов (таймауты 30с на любых страницах) — годен лишь для
  навигации и чтения DOM-текста. Сегодня перепроверено частично: навигация и `get_page_text`
  на `:8791` отрабатывают штатно; **скриншот измерить не удалось** — в неинтерактивной сессии
  панель не отображается и не композитит кадры («Screenshot timed out after 5s: the Browser
  pane is not displayed»), это ДРУГАЯ ошибка, не те 30с. Правило оставлено в силе; чтобы снять
  или подтвердить его, нужен один замер в живой сессии с открытой панелью. Учти при замере:
  MCP-вызовы дольше 2 минут теперь авто-уходят в фон (CC 2.1.212), так что «вечное ожидание»
  больше не съедает ход.
  ...
  use» — заново добавить `--isolated` в args плагина playwright (args живут в конфиге плагина,
  вне репозитория, — в `.claude/settings.local.json` их нет).
...
- **Авто-память MEMORY.md — append-only:** новая заметка = отдельный файл + СТРОКА В КОНЕЦ
  индекса; не переписывать чужие записи — файл общий для параллельных сессий. Правило про
  append-only относится к ТЕЛУ заметок и строкам индекса: frontmatter (в т.ч. ISO-поле
  `modified`, появилось в CC 2.1.214) ведёт сам Claude Code — его изменение не является чужой
  правкой и чинить его руками не надо.
...
- **Гигиена сессий:** ≥2 компакций или ≥3 дня жизни сессии → закрыть на ближайшей вехе
  ...
  мега-сессии дали 91% всего расхода cache-read за неделю аудита. **1M контекста у Opus 5
  правило не отменяет** — cache-read платится за каждый ход. Отпочковать побочную ветку
  работы без потери контекста — `/fork` (копия диалога в фоновую сессию), вернуть утраченное —
  `/rewind` (умеет и до `/clear`). Подробности — [PLAYBOOK.md §5](coordination/PLAYBOOK.md).
- **Dev-серверы — только через launch.json / `scripts/dev-servers.ps1`** (идемпотентный, health-poll;
  `-Restart`, `-Demo`). Причина уже не «фоновое умрёт на границе хода» — фоновый Bash
  (`run_in_background`) теперь живёт между ходами и сам сообщает о завершении. Причина в другом:
  скрипт идемпотентен, ждёт health и не плодит дубли на порту, а ad-hoc процесс приходится
  помнить и гасить руками;
