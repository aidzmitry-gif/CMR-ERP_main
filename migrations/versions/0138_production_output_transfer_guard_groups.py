...
BEGIN
  ...
  IF (NEW.basis->>'organization_id') IS DISTINCT FROM NEW.organization_id::text
     OR (NEW.basis->>'policy_id') IS DISTINCT FROM e.policy_id::text
     OR (NEW.posting->>'policy_id') IS DISTINCT FROM e.policy_id::text
     OR NEW.basis->'target'->>'finished_goods_account' IS NULL
     OR NEW.basis->'wip'->>'account' IS NULL
     OR NEW.basis->>'candidate_transfer_byn' IS NULL
     OR NEW.basis->'output'->>'accepted_quantity' IS NULL THEN
  ...
  SELECT count(*) FILTER (WHERE line->>'side'='debit'), count(*) FILTER (WHERE line->>'side'='credit')
    INTO declared_debit_count, declared_credit_count FROM jsonb_array_elements(NEW.posting->'lines') line;
  declared_line_count := jsonb_array_length(NEW.posting->'lines');

  SELECT * INTO actual_debit FROM accounting.line
    WHERE entry_id=NEW.entry_id AND side='debit';
  SELECT line INTO declared_debit FROM jsonb_array_elements(NEW.posting->'lines') line
  ...
  IF debit_count <> 1 OR declared_debit_count <> 1 OR declared_debit IS NULL
     OR actual_debit.account_code IS DISTINCT FROM NEW.basis->'target'->>'finished_goods_account'
     OR actual_debit.currency IS DISTINCT FROM 'BYN'
     OR actual_debit.amount IS DISTINCT FROM (NEW.basis->>'candidate_transfer_byn')::numeric
     OR actual_debit.quantity IS DISTINCT FROM (NEW.basis->'output'->>'accepted_quantity')::numeric
  ...
  IF NOT (NEW.basis->'wip' ? 'groups') THEN
  ...
  SELECT count(*), coalesce(sum((item->>'balance_byn')::numeric),0),
         ...
         coalesce(jsonb_agg(jsonb_build_object('account',NEW.basis->'wip'->>'account','side','credit',
    ...
    FROM jsonb_array_elements(NEW.basis->'wip'->'groups') item
  ...
  IF expected_group_count < 1 OR expected_group_count <> debit_count
     ...
     OR jsonb_array_length(NEW.basis->'wip'->'groups') <> expected_group_count THEN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('account',line->>'account','side',line->>'side',
    ...
    INTO declared_credits FROM jsonb_array_elements(NEW.posting->'lines') line
