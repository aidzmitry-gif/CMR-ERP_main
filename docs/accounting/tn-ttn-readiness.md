...
Для каждого юрлица и периода должна существовать отдельная, версионируемая запись политики отгрузочных документов. До её заполнения система может показывать только черновик.
...
## Минимальная приёмка будущего вертикального среза

1. Бухгалтер создаёт policy без предустановленной формы и без тестовых реквизитов.
2. Из подтверждённого WMS-акта строится черновик; без policy, формы, реквизитов, номера или полномочий кнопка выпуска заблокирована с объяснением причины.
3. В тестовом контуре один выпуск связывается с неизменяемым WMS-актом и версией policy; повтор с тем же request key не создаёт второй номер.
4. Подписанная электронная накладная не редактируется и не отменяется API-командой; тест доказывает только создание связанного корректирующего/возвратного документа.
5. В реестре сделки видны WMS-акт, подготовленный документ, внешний идентификатор/статус и цепочка корректировки. Внешняя отправка выполняется только уполномоченным пользователем после отдельной проверки.
