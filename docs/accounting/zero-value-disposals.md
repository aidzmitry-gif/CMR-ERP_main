...
## Linked zero-cost sales (migration 0143, local acceptance)
The existing sales preview/confirm routes now support a specific-cost finished-goods lot with one authenticated production-output origin and zero book value. Revenue and VAT remain real monetary Entry lines. A linked immutable V3 quantity command includes the complete sale document and uses the sale Entry id as its registration token. PostgreSQL rejects a missing quantity receipt, a foreign-organization entry, inconsistent monetary amounts or VAT analytics, and incomplete reciprocal bindings at transaction commit. No zero-money COGS lines are created.
Late production costs include these linked sales: selling0.5 of2 units at zero cost and subsequently allocating100BYN produces75BYN on the remaining stock and25BYN on90.4. Historical sale verification excludes its own quantity event and retains its original calculation after the later cost revision. Legacy V1/V2 issue commands and hashes remain unchanged. Downgrade0143 refuses persisted zero-sale history.
Local evidence covers actual HTTP preview/concurrent confirm for VAT0 and20, exact retries, late-cost allocation, historical verification, rollback of incomplete/forged packages, cross-organization rejection, migration roundtrip and legacy history preservation. Browser rendering, deployment and statutory monthly closing are separate acceptance steps. FIFO/weighted/multiple-origin zero disposals remain outstanding; this extension does not claim their completion.
