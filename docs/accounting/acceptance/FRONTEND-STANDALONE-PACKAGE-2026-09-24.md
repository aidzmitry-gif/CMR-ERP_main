...
затем тот же архив заново распакован в отдельном Linux-контейнере с **точной
серверной Node v22.22.3** (`node@sha256:e21fc383b50d5347dc7a9f1cae45b8f4e2f0d39f7ade28e4eef7d2934522b752`).
При `--network none` и read-only root `node server.js` сообщил Ready,
`BUILD_ID` совпал, анонимный бухгалтерский маршрут направил на login,
статический JS вернул 200. Временный контейнер удалён. Совместимость Node
patch в этой локальной среде подтверждена; systemd switch и rollback на
сервере ещё не проверены. Оба архива остались локальными; на сервер ничего
не отправлялось.
